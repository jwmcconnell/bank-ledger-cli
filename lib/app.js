...
const checkBalance = () => {
  return getAgent()
    ...
    .then(res => {
      console.log(chalk.green('Balance:', res.body.balance.toFixed(2)));
      return ledger();
    });
};
...
const getTransactions = () => {
  return getAgent()
    ...
    .then(res => {
      if(res.body.length) {
        const formatedTransactions = res.body.map(transaction => ({
          'Starting Balance': transaction.startingBalance.toFixed(2),
          'Amount': transaction.amount.toFixed(2),
          'Type': transaction.type,
          'Ending Balance': transaction.endingBalance.toFixed(2),
          'Date': transaction.date
        }));
        console.table(formatedTransactions);
      }
